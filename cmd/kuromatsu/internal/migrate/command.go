...
import (
	...
	"github.com/andre25costa-code/kuromatsu/pkg/migrate"
)
...
func NewMigrateCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "migrate",
		Short: "Migrate from xxxclaw(openclaw, etc.) to kuromatsu",
		Args:  cobra.NoArgs,
		Example: `  kuromatsu migrate
  kuromatsu migrate --from openclaw
  kuromatsu migrate --dry-run
  kuromatsu migrate --refresh
  kuromatsu migrate --force`,
	...
	cmd.Flags().StringVar(&opts.TargetHome, "target-home", "",
		"Override target home directory (default: ~/.kuromatsu)")
