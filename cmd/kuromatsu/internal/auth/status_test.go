...
import (
	...
	pkgauth "github.com/andre25costa-code/kuromatsu/pkg/auth"
...
func setAuthStatusTestHome(t *testing.T) string {
	...
	tmpDir := t.TempDir()
	t.Setenv(config.EnvHome, filepath.Join(tmpDir, ".kuromatsu"))
	return tmpDir
}
...
func TestAuthStatusCmdShowsCanonicalGoogleAntigravityAfterLegacyRefresh(t *testing.T) {
	...
	authPath := filepath.Join(tmpDir, ".kuromatsu", "auth.json")
