...
func TestResolve_EncKey_RoundTrip(t *testing.T) {
	...
	t.Setenv("KUROMATSU_SSH_KEY_PATH", sshKeyPath)
	...
	t.Setenv("KUROMATSU_KEY_PASSPHRASE", passphrase)
...
func TestResolve_EncKey_WithSSHKey(t *testing.T) {
	...
	// Set KUROMATSU_SSH_KEY_PATH before Encrypt so the path passes allowedSSHKeyPath validation.
	t.Setenv("KUROMATSU_KEY_PASSPHRASE", passphrase)
	t.Setenv("KUROMATSU_SSH_KEY_PATH", sshKeyPath)
...
func TestResolve_EncKey_NoPassphrase(t *testing.T) {
	...
	t.Setenv("KUROMATSU_KEY_PASSPHRASE", "")

	r := credential.NewResolver(t.TempDir())
	_, err = r.Resolve(enc)
	if err == nil {
		t.Fatal("expected error when KUROMATSU_KEY_PASSPHRASE is unset, got nil")
	}
}

func TestResolve_EncKey_BadCiphertext(t *testing.T) {
	t.Setenv("KUROMATSU_KEY_PASSPHRASE", "some-passphrase")
	t.Setenv("KUROMATSU_SSH_KEY_PATH", "")
...
func TestResolve_EncKey_WrongPassphrase(t *testing.T) {
	...
	t.Setenv("KUROMATSU_KEY_PASSPHRASE", "wrong-passphrase")
...
func TestDeriveKey_SSHKeyNotFound(t *testing.T) {
	...
	t.Setenv("KUROMATSU_KEY_PASSPHRASE", "passphrase")
	t.Setenv("KUROMATSU_SSH_KEY_PATH", filepath.Join(dir, "nonexistent_key"))
...
// that are not under KUROMATSU_SSH_KEY_PATH, KUROMATSU_HOME, or ~/.ssh/.
