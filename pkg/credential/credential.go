...
//   - Encrypted:   "enc://<base64>"     → AES-256-GCM decrypt via KUROMATSU_KEY_PASSPHRASE
...
//  2. KUROMATSU_SSH_KEY_PATH env var
...
const PassphraseEnvVar = "KUROMATSU_KEY_PASSPHRASE"

// PassphraseProvider is the function used to retrieve the passphrase for enc://
// credential decryption. It defaults to reading KUROMATSU_KEY_PASSPHRASE from the
...
const SSHKeyPathEnvVar = "KUROMATSU_SSH_KEY_PATH"
...
const picoclawHome = "KUROMATSU_HOME"
...
//
// passphrase is required (KUROMATSU_KEY_PASSPHRASE value).
// sshKeyPath is the SSH private key file to use; pass "" to auto-detect via
// KUROMATSU_SSH_KEY_PATH env var or ~/.ssh/picoclaw_ed25519.key.
...
//   - exact match with KUROMATSU_SSH_KEY_PATH env var
//   - within the KUROMATSU_HOME env var directory
//   - within ~/.ssh/
func allowedSSHKeyPath(path string) bool {
	...
	// Exact match with KUROMATSU_SSH_KEY_PATH.
	...
	if sshKeyPath == "" {
		return nil, fmt.Errorf(
			"credential: SSH private key is required but not found" +
				" (set KUROMATSU_SSH_KEY_PATH or place key at ~/.ssh/picoclaw_ed25519.key)")
	}
	if !allowedSSHKeyPath(sshKeyPath) {
		return nil, fmt.Errorf(
			"credential: SSH key path %q is not in an allowed location (KUROMATSU_SSH_KEY_PATH, KUROMATSU_HOME, or ~/.ssh/)",
			sshKeyPath,
		)
	}
