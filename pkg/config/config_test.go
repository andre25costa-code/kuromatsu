...
// KUROMATSU_SSH_KEY_PATH to its path for the duration of the test. This is required
...
func mustSetupSSHKey(t *testing.T) {
	...
	t.Setenv("KUROMATSU_SSH_KEY_PATH", keyPath)
}
...
func TestDefaultConfig_WorkspacePath_WithPicoclawHome(t *testing.T) {
	...
	if cfg.Agents.Defaults.Workspace != want {
		t.Errorf(
			"Workspace path with KUROMATSU_HOME = %q, want %q",
...
func TestLoadConfig_WarnsForPlaintextAPIKey(t *testing.T) {
	...
	t.Setenv("KUROMATSU_KEY_PASSPHRASE", "test-passphrase")
...
// unchanged when KUROMATSU_KEY_PASSPHRASE is not set.
...
// TestLoadConfig_MixedKeys_NoPassphrase verifies that when KUROMATSU_KEY_PASSPHRASE
