...
type GatewayConfig struct {
	Host      string `json:"host"                env:"KUROMATSU_GATEWAY_HOST"`
	Port      int    `json:"port"                env:"KUROMATSU_GATEWAY_PORT"`
	HotReload bool   `json:"hot_reload"          env:"KUROMATSU_GATEWAY_HOT_RELOAD"`
	LogLevel  string `json:"log_level,omitempty" env:"KUROMATSU_LOG_LEVEL"`
}
...
// The KUROMATSU_LOG_LEVEL environment variable overrides the file value.
func ResolveGatewayLogLevel(path string) string {
	...
	if envLevel := os.Getenv("KUROMATSU_LOG_LEVEL"); envLevel != "" {
