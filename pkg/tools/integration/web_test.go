...
// with cf-mitigated: challenge triggers a retry using the honest kuromatsu User-Agent,
...
func TestWebFetchTool_CloudflareChallenge_RetryWithHonestUA(t *testing.T) {
	...
	// Second request must use the honest kuromatsu user agent
	if !strings.Contains(receivedUAs[1], "kuromatsu") {
		t.Errorf("retry request UA = %q, want it to contain 'kuromatsu'", receivedUAs[1])
	}
}
